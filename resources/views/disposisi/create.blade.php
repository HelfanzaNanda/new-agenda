					...
					<div class="row">
						<div class="col-md-6">	
							...
							<div class="form-group">
								...
								<select name="agenda" id="input-agenda" class="form-control {{ $agenda_id ? '' : 'select-single' }}"
								{{ $agenda_id ? 'readonly' : '' }}>
									<option value="" selected disabled>Pilih Agenda</option>
									@foreach ($agendas as $agenda)
										<option value="{{ $agenda->id }}"
											{{ $agenda_id ? ($agenda_id == $agenda->id ? 'selected' : '') : '' }}>
											{{ $agenda->kegiatan }}
										</option>
									@endforeach
								</select>
...
@push('scripts')
	<script>
		...
		async function generateNumber() {  
			try {
				const url = "{{ route('number.generate') }}"+"?prefix=DISPOSISI"
				const response = await axios.get(url)
				$('#input-no-surat').val(response.data.number)
			} catch (error) {
		...
		$(document).on('click', '.btn-save', async function (e) {  
			...
			const url = "{{ route('disposisi.store') }}"
			try {
